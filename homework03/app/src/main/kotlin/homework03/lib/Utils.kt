...
class Utils {
    companion object {
        ...
        fun getLinear(commentSnapshotList: List<CommentSnapshot>?, discussionId: String?): List<LinearComment> {
            ...
            commentSnapshotList?.forEach {
                res.add(toLinearComment(it, discussionId)); res.addAll(
                getLinear(
                    it.children,
                    discussionId
                )
        ...
        fun getDiscussionLinks(topicSnapshot: TopicSnapshot): List<Pair<String?, String>>? =
