...
sealed class FList<T> : Iterable<T> {
    ...
    data class Nil<T>(private val dummy: Int = 0) : FList<T>() {
        ...
        override fun iterator(): Iterator<T> = object : Iterator<T> {
            ...
            override fun next(): T = throw NoSuchElementException("FList if empty")
        }
    }
    ...
    data class Cons<T>(val head: T, val tail: FList<T>) : FList<T>() {
        ...
        override fun iterator(): Iterator<T> = object : Iterator<T> {
            var currentElement: FList<T> = this@Cons

            override fun hasNext(): Boolean = !currentElement.isEmpty

            override fun next(): T {
                if (currentElement.isEmpty) throw NoSuchElementException("FList if empty")
                val value = (currentElement as Cons<T>).head
                currentElement = (currentElement as Cons<T>).tail
                return value
            }
        }
    }
...
fun <T> flistOf(vararg values: T): FList<T> =
    ...
    else FList.Cons(values[0], flistOf(*values.sliceArray(1 until values.size)))
